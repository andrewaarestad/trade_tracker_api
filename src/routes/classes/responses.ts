...
export class Responses {
  ...
  public static async sendOkResponse(req: e.Request, res: e.Response, params?: IOkParams) {
    if (!params) {
      ...
      // await ApiLoggingService.logApiRequest(req, 204);
      ...
      // await ApiLoggingService.logApiRequest(req, code, responseBody);
  ...
  public static async sendErrorResponse(req: e.Request, res: e.Response, error?: DomainError | Error) {
    ...
    if (!error) {
      ...
      if (!errorIsCircular) {
        error = new InternalServerError('Invalid error: ' + error + ', ' + error.constructor.name + ', ' + JSON.stringify(error));
      } else {
    ...
    // await ApiLoggingService.logApiRequest(req, code, responseBody, errorParams.meta, error, stack);
